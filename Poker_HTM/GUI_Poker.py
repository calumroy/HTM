        ...
        for lev in range(0,self.numLevels):
            ...
            if self.iteration%twoPowLev==0:
                ...
                if lev!=0:
                    ...
                    print "     current pred command=%s"%(pred_command)
                    self.command[lev] = pred_command
                    if self.command[lev]=='none':
                        self.command[lev]=pkcd.randomCommand() 
                        print "     fbComm is none chose %s"%self.command[higherLevel]
                ...
                if lev<(self.numLevels-1):
                    ...
                    upperCommInput=pkcd.newCommInput(self.comSpaceWidth,self.numCommRows,self.command[higherLevel])
                else:
